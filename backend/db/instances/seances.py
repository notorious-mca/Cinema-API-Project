...
def update_seance_by_id(id:int, seance: SeanceCreate,db: Session,owner_id):
    ...
    existing_film = db.query(Film).filter(Film.id == seance.film_id)
    if not existing_film.first():
        return -1
    seance.__dict__.update(owner_id=owner_id)  #update dictionary with new key value of owner_id
